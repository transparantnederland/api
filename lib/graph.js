...
var orgsFromOrgQuery = fs.readFileSync(path.join(__dirname, '..', 'queries', 'orgsFromOrg.cypher'), { encoding: 'utf8' });
...
exports.orgsFromOrg = function (id, relations, callback) {
  var query = replaceRelations(orgsFromOrgQuery, relations);
  ...
  graphDb.cypher({
    ...
    callback(err, (results || []).reduce(function (response, result) {
      response.ids.push(result.id);
      response.types.push(result.type);
      response.to.push(result.relation_id);
      return response;
    }, {
      ids: [],
      types: [],
...
exports.orgsFromPerson = function (id, relations, organizations, callback) {
  ...
  graphDb.cypher({
    ...
    if (results) {
      ...
      to = results.map(function (result) {
        return result['to'];
      });
    }
    // return both ids and the type of relations
    callback(err, { 'ids': ids, 'types': types, 'since': since, 'until': until, 'to': to });
  });
};
